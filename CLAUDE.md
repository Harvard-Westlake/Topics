...
- **Year Schedule blocks show a Canvas sync-status dot** next to each Sync button, checked against the course that schedule last synced to (`syncCourseId_<schedule>` in localStorage): green ✓ = "it seems to be synced" (the expected module and all its items exist with matching titles, points, and due dates), yellow ● = partial match (module exists but items are missing or points/dates drifted — the tooltip says exactly what), ↑ = ready to sync (nothing on Canvas yet). Served by `/api/schedules/<name>/sync-status` (`_expected_block_items` mirrors the sync-block create loop), reads through the normal Canvas caches, and re-checks after every save and (live) after every sync.
...
- **Give each activity a diagram.** A small SVG in the lesson's `assets/` (dark self-backgrounded, using the repo's colors) embedded right after the `*Concept:*` line with full descriptive alt text, showing the effect the student is about to cause — commit graphs for Git activities, data-flow for hashing, etc.
