...
> **Note:**
> For the terminal git check, typing `git` with no arguments should print a usage summary starting with `usage: git` — this confirms it is installed. If you see `no repository` instead, that also confirms git is found (it just means you are not inside a git folder).
