...
1. **Run the full `Tester`.** It prints all three attention matrices for the three-card fixture, the uniform-versus-dot-product comparison at position 2, the Sample A retrieval run, and the future-change demonstration.
2. **Write your own `ScoreRule`.** The provided `SAMPLE_RULE` scores the retrieval sequence with +3 for a matching sample identifier, +1 for a matching category, minus a distance penalty. Write a second rule of your own design that still concentrates the earlier-position weight on the `Sample A` cards, and be ready to defend your point values — there is no single correct rule, which is exactly the point of Stage 2.
...
4. **The bridge question.** `BASALT` is token 301 and `LIMESTONE` is token 17. Explain why `301 * 17` is meaningless (cite Chapter 1's misconception) and what a feature vector provides that an identifier cannot.
...
- [ ] **The spotlight beats the blur** — the printed comparison shows `dark` at 0.667 (uniform) versus 0.767 (dot product) at position 2.
- [ ] **Retrieval succeeds** — rule-based attention on the retrieval fixture concentrates earlier-position weight on the `Sample A` cards, under both `SAMPLE_RULE` and your own rule.
...
Dot product BASALT . LIMESTONE (4 slots):
...
Dark feature at position 2 (uniform / dot product):
